 ...
 */
public abstract class BaseController<T> implements Runnable {
	...
	/**
	 * <pre>
	 * this is now protected. You can set this directly in
	 * your getMove() method to save an immediate response.
	 * </pre>
	 */
	protected T lastMove;

	/**
	 * Instantiates a new controller. The constructor initializes the class
	 * variables.
	 */
	 ...
	 */
	public final void update(GameEngine engine, long timeDue) {
		synchronized (this) {
			this.engine = engine;
				...
				if (!threadStillRunning) {
					new Thread() {
						public void run() {
							...
							lastMove = getMove(engine, timeDue);
							hasComputed = true;
							threadStillRunning = false;
						}
					}.start();
	 ...
	 *         controller)
	 */
	public abstract T getMove(GameEngine engine, long timeDue);
}