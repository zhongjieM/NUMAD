...
import edu.neu.madcourse.zhongjiemao.exerpacman.models.Ghost;
import edu.neu.madcourse.zhongjiemao.exerpacman.models.Maze;
import edu.neu.madcourse.zhongjiemao.exerpacman.models.Node;
import edu.neu.madcourse.zhongjiemao.exerpacman.models.Pacman;
import edu.neu.madcourse.zhongjiemao.exerpacman.models.PathsCache;

/**
 * Game computation class
 */
public final class GameEngine {
	private static final String TAG = "EXER_PACMAN_GAME_ENGINE";
	...
	private int mazeIndex, levelCount, currentLevelTime, totalTime, preTotalTime, score,
			ghostEatMultiplier, timeOfLastGlobalReversal;
	...
	/**
	 * Instantiates a new game. The seed is used to initialize the pseudo-random
	 ...
	 */
	public GameEngine(long seed, Context context, Handler handler) {
		mGame = (ExerPacmanGame) context;
		mHandler = handler;
		this.seed = seed;
		rnd = new Random(seed);
		mazes = new Maze(0, mGame);
		_init(0);
	}
	 ...
	 */
	public GameEngine(long seed, int initialMaze, Context context, Handler handler) {
		mGame = (ExerPacmanGame) context;
		mHandler = handler;
		this.seed = seed;
		rnd = new Random(seed);
		mazes = new Maze(initialMaze, mGame);
		_init(initialMaze);
	}
	...
	private void _newLevelReset() {
		...
		mazes = new Maze(mazeIndex, mGame);
		currentMaze = mazes;
		...
		_levelReset();
		sendMessage(MSG_UPDATE_LEVEL_LOCK, mazeIndex, -1, null);
	}
	...
	private void _setPills(Maze maze) {
		...
		// pillsIndices.length = 220
		// reduce amount of pills
		List<Integer> tmp = generatePillsIndices();
		for (Integer i : tmp) {
			pills.set(i);
		}
//		pills.set(0, currentMaze.pillIndices.length);
		powerPills = new BitSet(currentMaze.powerPillIndices.length);
		// powerPillsIndices.length = 4
		...
		for (GHOST ghostType : GHOST.values())
			ghosts.put(
					...
					new Ghost(ghostType, currentMaze.lairNodeIndex, 0,
							(int) (ghostType.initialLairTime * (Math.pow(
									LAIR_REDUCTION, levelCount
	...
	public String getGameState() {
		...
		sb.append(mazeIndex + "," + totalTime + "," + score + ","
				+ currentLevelTime + "," + levelCount + ","
				+ pacman.currentNodeIndex + "," + pacman.lastMoveMade + ","
				+ pacman.numberOfLivesRemaining + ","
				+ pacman.hasReceivedExtraLife + ",");

		for (Ghost ghost : ghosts.values())
			sb.append(ghost.currentNodeIndex + "," + ghost.edibleTime + ","
					+ ghost.lairTime + "," + ghost.lastMoveMade + ",");
	...
	public void setGameState(String gameState) {
		...
		int index = 0;
		gameOver = Boolean.parseBoolean(values[index++]);
		...
		pacman = new Pacman(Integer.parseInt(values[index++]),
		...
		for (GHOST ghostType : GHOST.values())
			ghosts.put(
					ghostType,
					new Ghost(ghostType, Integer.parseInt(values[index++]),
							Integer.parseInt(values[index++]), Integer
									.parseInt(values[index++]), MOVE
...
//				pills.clear(i);
	...
	public void advanceGameWithoutReverse(MOVE pacManMove,
	...
	public void advanceGameWithForcedReverse(MOVE pacManMove,
			EnumMap<GHOST, MOVE> ghostMoves) {
		updatePacman(pacManMove);
	...
	public void advanceGameWithPowerPillReverseOnly(MOVE pacManMove,
			EnumMap<GHOST, MOVE> ghostMoves) {
		updatePacman(pacManMove);
	...
	public void updatePacman(MOVE pacManMove) {
		// show hint
		checkJunctions(pacManMove);
		// move pac-man
		_updatePacman(pacManMove);
		// eat a pill
		_eatPill();
		// eat a power pill
		_eatPowerPill();
	...
	private void checkJunctions(MOVE pacManMove) {
		// if not using hints or in swipe mode, don't show the hint
		if (!USE_ROTATION_HINT || IS_SWIPE_MODE)
			return;
		int nodeIndex = getPacmanCurrentNodeIndex();
		int neighbourIndex = getFarNeighbour(nodeIndex, pacManMove, 30);
		if (neighbourIndex != -1) {
			MOVE[] moves = getPossibleMoves(neighbourIndex, pacManMove);
//			Log.d(TAG, moves.length + " possible moves");
			if (moves.length == 3) {
				// TURN_BOTH
				sendMessage(MSG_SHOW_JUNCTION_HINT, TURN_BOTH, -1, null);
			} else if (moves.length == 1) {
				if (!moves[0].equals(pacManMove) && moves[0].equals(pacManMove.left())) {
					// TURN_LEFT
					sendMessage(MSG_SHOW_JUNCTION_HINT, TURN_LEFT, -1, null);
				} else if (!moves[0].equals(pacManMove) && moves[0].equals(pacManMove.right())) {
					// TURN_RIGHT
					sendMessage(MSG_SHOW_JUNCTION_HINT, TURN_RIGHT, -1, null);
				} else {
					// TURN_NONE
					sendMessage(MSG_SHOW_JUNCTION_HINT, TURN_NONE, -1, null);
				}
			} else {
				if (!moves[0].equals(pacManMove) && !moves[1].equals(pacManMove)) {
					// TURN_BOTH
					sendMessage(MSG_SHOW_JUNCTION_HINT, TURN_BOTH, -1, null);
				} else if (moves[0].equals(pacManMove)) {
					if (moves[1].equals(pacManMove.left())) {
						// TURN_LEFT
						sendMessage(MSG_SHOW_JUNCTION_HINT, TURN_LEFT, -1, null);
					} else {
						// TURN_RIGHT
					...
					if (moves[0].equals(pacManMove.left())) {
						// TURN_LEFT
						sendMessage(MSG_SHOW_JUNCTION_HINT, TURN_LEFT, -1, null);
					} else {
						// TURN_RIGHT
	...
	public int getFarNeighbour(int nodeIndex, MOVE moveToBeMade, int steps) {
		int tmpNeighbourIndex = nodeIndex;
		for (int i = 0; i < steps; i++) {
			Integer nextNeighbourIndex = currentMaze.graph[tmpNeighbourIndex].neighbourhood.get(moveToBeMade);
			if (nextNeighbourIndex != null) {
				tmpNeighbourIndex = nextNeighbourIndex;
				MOVE[] moves = getPossibleMoves(tmpNeighbourIndex, moveToBeMade);
				if (moves.length > 1) {
					return tmpNeighbourIndex;
				} else if (!moves[0].equals(moveToBeMade)) {
					return tmpNeighbourIndex;
				} else {
					continue;
				}
	...
	public void updateGame() {
		...
		totalTime++;
		currentLevelTime++;
		int tmpTime = (totalTime * DELAY) / 1000;
		if (tmpTime > preTotalTime) {
			preTotalTime = tmpTime;
			int timeLeft = MAX_TIME_SEC - tmpTime;
			sendMessage(MSG_UPDATE_TIME, timeLeft, -1, null);
		}
	 ...
	 */
	public void updateGame(boolean feast, boolean updateLairTimes,
			boolean updateExtraLife, boolean updateTotalTime,
	...
	private void _updatePacmanExtraLife() {
		...
		if (!pacman.hasReceivedExtraLife && score >= EXTRA_LIFE_SCORE) {
	...
	private void _updatePacman(MOVE move) {
		...
		pacman.currentNodeIndex = pacman.lastMoveMade == MOVE.NEUTRAL ? pacman.currentNodeIndex
				: currentMaze.graph[pacman.currentNodeIndex].neighbourhood
						.get(pacman.lastMoveMade);
	}
		...
		for (Entry<GHOST, MOVE> entry : moves.entrySet()) {
			...
			if (ghost.lairTime == 0) {
				if (ghost.edibleTime == 0
						|| ghost.edibleTime % GHOST_SPEED_REDUCTION != 0) {
	...
	private MOVE _checkGhostDir(Ghost ghost, MOVE direction) {
		...
		// of that ghost
		if (node.neighbourhood.containsKey(direction)
				&& direction != ghost.lastMoveMade.opposite())
			return direction;
		else {
	...
	private void _eatPill() {
		...
		if (pillIndex >= 0 && pills.get(pillIndex) && squatted) {
			score += PILL;
			pills.clear(pillIndex);
			pillWasEaten = true;
			squatted = false;
			sendMessage(MSG_UPDATE_SCORE, score, SUB_MSG_EAT_FRUIT, null);
		}
	}
	...
	private void _eatPowerPill() {
		...
		if (powerPillIndex >= 0 && powerPills.get(powerPillIndex) && squatted) {
			score += POWER_PILL;
			ghostEatMultiplier = 1;
			...
			squatted = false;
			sendMessage(MSG_UPDATE_SCORE, score, SUB_MSG_EAT_SWORD, null);
		}
	}
	...
	private boolean _reverseGhosts(EnumMap<GHOST, MOVE> moves, boolean force) {
		...
		for (Entry<GHOST, MOVE> entry : moves.entrySet()) {
			...
			if (currentLevelTime > 1 && ghost.lairTime == 0
	...
	private void _feast() {
		...
		for (GHOST ghost : GHOST.values())
			ghostsEaten.put(ghost, false);
		for (Ghost ghost : ghosts.values()) {
			int distance = getShortestPathDistance(pacman.currentNodeIndex,
			...
			if (distance <= EAT_DISTANCE && distance != -1) {
				if (ghost.edibleTime > 0) {
					...
					sendMessage(MSG_UPDATE_SCORE, score, SUB_MSG_EAT_GHOST, null);
				} else {
					// ghost eats pacman
					pacman.numberOfLivesRemaining--;
					int tmpPacmanLives = pacman.numberOfLivesRemaining;
					pacmanWasEaten = true;
					if (pacman.numberOfLivesRemaining <= 0) {
						gameOver = true;
						sendMessage(MSG_SHOW_GAME_OVER, SUB_MSG_GAME_OVER_DIE, -1, null);
					} else {
						_deductLives(tmpPacmanLives);
//						_levelReset();
					}
					return;
				}
	...
	private void _deductLives(int lives) {
		ghostEatMultiplier = 1;
		mGame.vibrate();
		sendMessage(MSG_UPDATE_LIVES, lives, SUB_MSG_LOSE_LIVES, null);
	}
	...
	private void _checkLevelState() {
		...
		if (totalTime * DELAY + 1 > MAX_TIME) {
			gameOver = true;
			// TODO can recover here
//			score += pacman.numberOfLivesRemaining * AWARD_LIFE_LEFT;
			sendMessage(MSG_SHOW_GAME_OVER, SUB_MSG_GAME_OVER_TIME_UP, -1, null);
		}
		// if all pills have been eaten or the time is up...
		else if ((pills.isEmpty() && powerPills.isEmpty())
				|| currentLevelTime >= LEVEL_LIMIT)
			_newLevelReset();
	}
	...
	public boolean doesGhostRequireAction(GHOST ghostType) {
		...
		return ((isJunction(ghosts.get(ghostType).currentNodeIndex) || (ghosts
				.get(ghostType).lastMoveMade == MOVE.NEUTRAL)
	...
	public int[] getNeighbouringNodes(int nodeIndex) {
		return currentMaze.graph[nodeIndex].allNeighbouringNodes
				.get(MOVE.NEUTRAL);
	}
	...
	public int getNeighbour(int nodeIndex, MOVE moveToBeMade) {
		Integer neighbour = currentMaze.graph[nodeIndex].neighbourhood
				.get(moveToBeMade);
	 ...
	 */
	public MOVE getMoveToMakeToReachDirectNeighbour(int currentNodeIndex,
			int neighbourNodeIndex) {
		for (MOVE move : MOVE.values()) {
			if (currentMaze.graph[currentNodeIndex].neighbourhood
					.containsKey(move)
					&& currentMaze.graph[currentNodeIndex].neighbourhood
							.get(move) == neighbourNodeIndex) {
				return move;
			}
		}
	...
	private List<Integer> generatePillsIndices() {
		List<Integer> integers = new ArrayList<Integer>();
		Random random = new Random(System.nanoTime());
		while (true) {
			if (integers.size() >= NUM_PILLS[mazeIndex])
				break;
			int tmp = random.nextInt(220);
	...
	/**
	 * Returns the PATH distance from any node to any other node.(KEEP)
	 * 
	 * @param fromNodeIndex
	 *            the from node index
	 * @param toNodeIndex
	...
	public double getEuclideanDistance(int fromNodeIndex, int toNodeIndex) {
		return Math.sqrt(Math.pow(currentMaze.graph[fromNodeIndex].x
	...
	public int getManhattanDistance(int fromNodeIndex, int toNodeIndex) {
		return (int) (Math.abs(currentMaze.graph[fromNodeIndex].x
				- currentMaze.graph[toNodeIndex].x) + Math
				.abs(currentMaze.graph[fromNodeIndex].y
	...
	public double getDistance(int fromNodeIndex, int toNodeIndex,
		...
		switch (distanceMeasure) {
		case PATH:
			return getApproximateShortestPathDistance(fromNodeIndex,
	 ...
	 */
	public int getClosestNodeIndexFromNodeIndex(int fromNodeIndex,
			int[] targetNodeIndices, DM distanceMeasure) {
		...
		for (int i = 0; i < targetNodeIndices.length; i++) {
			...
			distance = getDistance(targetNodeIndices[i], fromNodeIndex,
					distanceMeasure);
	 ...
	 */
	public int getFarthestNodeIndexFromNodeIndex(int fromNodeIndex,
			int[] targetNodeIndices, DM distanceMeasure) {
	...
	/**
	 * Gets the next move towards target.(KEEP)
	 * 
	 * @param fromNodeIndex
	 *            the from node index
	 * @param toNodeIndex
	 ...
	 */
	public MOVE getNextMoveTowardsTarget(int fromNodeIndex, int toNodeIndex,
			DM distanceMeasure) {
		MOVE move = null;

		double minDistance = Integer.MAX_VALUE;

		for (Entry<MOVE, Integer> entry : currentMaze.graph[fromNodeIndex].neighbourhood
				.entrySet()) {
			double distance = getDistance(entry.getValue(), toNodeIndex,
					distanceMeasure);
	 ...
	 */
	public MOVE getNextMoveAwayFromTarget(int fromNodeIndex, int toNodeIndex,
			DM distanceMeasure) {
		MOVE move = null;
	 ...
	 */
	public MOVE getApproximateNextMoveTowardsTarget(int fromNodeIndex,
			int toNodeIndex, MOVE lastMoveMade, DM distanceMeasure) {
		MOVE move = null;

		double minDistance = Integer.MAX_VALUE;

		for (Entry<MOVE, Integer> entry : currentMaze.graph[fromNodeIndex].allNeighbourhoods
				.get(lastMoveMade).entrySet()) {
			double distance = getDistance(entry.getValue(), toNodeIndex,
					distanceMeasure);
	 ...
	 * @return The approximate next move towards target (chosen greedily)
	 */
	public MOVE getApproximateNextMoveAwayFromTarget(int fromNodeIndex,
			int toNodeIndex, MOVE lastMoveMade, DM distanceMeasure) {
		MOVE move = null;
	 ...
	 */
	@Deprecated
	public int getApproximateShortestPathDistance(int fromNodeIndex,
			int toNodeIndex, MOVE lastMoveMade) {
	...
	public int getShortestPathDistance(int fromNodeIndex, int toNodeIndex,
		...
		return caches[mazeIndex].getPathDistanceFromA2B(fromNodeIndex,
				toNodeIndex, lastMoveMade);
	}
}