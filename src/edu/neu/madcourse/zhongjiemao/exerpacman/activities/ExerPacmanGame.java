...
//	private WakeLock mWakeLock;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		ExerPacmanMusic.playOneByOne(this, R.raw.exer_pacman_beginning, R.raw.exer_pacman_bg);
	}
	...
	private void doPause(String pauseType) {
		...
		.setItems(R.array.exer_pacman_pause, new DialogInterface.OnClickListener() {
			@Override
			public void onClick(DialogInterface dialog, int which) {
				if (which == PAUSE_RESUME) {
					...
					ExerPacmanMusic.play(ExerPacmanGame.this, R.raw.exer_pacman_bg, true);
					mEngine.setGameState(INIT_GAME_STATE[mEngine.getMazeIndex()]);
					mGameView.getSurfaceDrawer().setPause(false);
	...
	private void setupScale() {
		Bitmap template = BitmapFactory.decodeResource(getResources(), R.drawable.maze_a);
...
//	}
	...
	private void initImageViews(ImageView left, ImageView right) {
		...
		Bitmap originLeftArrow = BitmapFactory.decodeResource(getResources(), R.drawable.arwl, options);
		Bitmap originRightArrow = BitmapFactory.decodeResource(getResources(), R.drawable.arwr, options);
		float ratio = ((float) (originLeftArrow.getHeight())) / ((float) (originLeftArrow.getWidth()));
		float dstW = ScaleUtil.getActualScreenW() / 6f;
		float dstH = dstW * ratio;
		Bitmap scaledLeftArrow = Bitmap.createScaledBitmap(originLeftArrow, (int) dstW, (int) dstH, true);
		Bitmap scaledRightArrow = Bitmap.createScaledBitmap(originRightArrow, (int) dstW, (int) dstH, true);
