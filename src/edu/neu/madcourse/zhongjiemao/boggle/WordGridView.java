...
public class WordGridView extends View {
	...
	protected void onMeasure(int width, int height) {
		...
		view_width = MeasureSpec.getSize(width);
		view_height = MeasureSpec.getSize(height);
		int small_side = (view_width < view_height ? view_width : view_height);
		view_width = small_side * 4 / 5;
		view_height = small_side;
		setMeasuredDimension(view_width, view_height);
		view_width = this.getWidth();
		view_height = this.getHeight();
		initializeCenterOfEachGrid();
		this.squareSideLength = this.getWidth() / 4;
		this.effectCenterRadius = this.squareSideLength * 3 / 4;
	}
	...
	public void gameOver(int over, int score, Stack<String> existed_words) {
		this.gamePaused = over;
		this.score = score;
		Object[] obj = existed_words.toArray();
		this.longestword = "";
		...
		for (int i = 0; i < obj.length; i++) {
			if (this.longestLength < String.valueOf(obj[i]).length()) {
				this.longestword = String.valueOf(obj[i]);
				this.longestLength = this.longestword.length();
			}
		}
		this.invalidate();
	}
	...
	private void initializeView() {
		...
		existed_letters = new boolean[16];
	...
	private void drawLetters(Canvas canvas, int gameStatus) {
		...
		switch (gameStatus) {
		case 1:
			// game pauses: draw pause
			foreground.setColor(Color.YELLOW);
			x = this.getWidth() / 2;
			y = this.getHeight() / 2 - (fm.ascent + fm.descent) / 2;
			canvas.drawText("Pause", x, y, foreground);
			break;
		case 2:
		...
		default:
			...
			for (int i = 0; i < 4; i++) {
				...
				for (int j = 0; j < 4; j++) {
					canvas.drawText(
							String.valueOf(boggleGame.letters[i * 4 + j]), j
	...
	private void drawSelection(Canvas canvas) {
		...
		for (int i = 0; i < v.length; i++) {
			...
			canvas.drawCircle(centerOfGrid_x[index / 4][index % 4],
					centerOfGrid_y[index / 4][index % 4],
	...
	private int getCharacter(float x, float y) {
		...
		if (index_x <= 3 && index_y <= 3) {
			if (Math.abs(x - centerOfGrid_x[index_y][index_x])
				...
				letter_index = index_x + index_y * 4;
		}
		return letter_index;
	}
	 ...
	 */
	private void initializeCenterOfEachGrid() {
